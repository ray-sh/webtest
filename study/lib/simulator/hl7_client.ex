defmodule HL7Client do
   ...
   def start_link(server_ip \\ @default_server, port \\ 5000, send_interval \\2) do
   ...
   def init([server_ip, port, send_interval]) do
      ...
      #Logger.info("The server info #{IO.inspect(server)}")
   ...
   def handle_info(:send_message, state) do
      if elem(state,2) do
         Logger.info("stat to send message to HL7 server")
         MLLP.Sender.async_send_message(elem(state,0), SimMessage.msg1())
      end
